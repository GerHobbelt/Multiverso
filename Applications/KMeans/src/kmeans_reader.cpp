...
/**************************** DataStore *****************************/
template <typename EleType>
void DataStore<EleType>::Read(int num_row,
        Sample<EleType>** buffer, SparseBlock<bool> *keys) {
    keys->Clear();
    int upper = num_row + current_read_;
    size_t index;
    for (int k = current_read_; k < upper; ++k) {
        Sample<EleType>& sample = buffer_[k%num_row_];
        buffer[k-current_read_] = &sample;
        for (int i = 0; i < sample.keys.size(); ++i) {
            index = sample.keys[i];
    ...
    current_read_ = upper % num_row_;
}

template <typename EleType>
void DataStore<EleType>::ReadAll(
        std::vector<Sample<EleType>>** buffer_ptr,
        SparseBlock<bool>** keys_ptr) {
    *buffer_ptr = &buffer_;
    *keys_ptr = &keys_;
}

template <typename EleType>
int DataStore<EleType>::Load() {
    TextReader reader(URI(file_), 1024);
    std::string line;
    int count = 0;
    Log::Write(Info, "line_offset_: %d, num_row_: %d\n", line_offset_, num_row_);
    for (int i = 0; i < line_offset_; ++i) {
        reader.GetLine(line);  // skip offset samples
    }
    while (reader.GetLine(line) && count < num_row_) {
        auto* sample = ParseLine(line);
        ++count;
    }
    Log::Write(Info,
        "\033[1;32m[Worker %d] Loaded %d samples.\033[0m\n",
        multiverso::MV_WorkerId(), count);
    return count;
}

template <typename EleType>
Sample<EleType>* DataStore<EleType>::ParseLine(const std::string& line) {
    this->buffer_.push_back(Sample<EleType>(row_size_, sparse_));
    Sample<EleType>* data = &(this->buffer_.back());
    std::stringstream iss(line);
    iss >> data->label;
    if (type_ == 0) {
        data->label = std::max(0, data->label);
    } else if (type_ == -1 && data->label == 0) {
        data->label = -1;
    }
...
DECLARE_TEMPLATE_CLASS_WITH_BASIC_TYPE(DataStore);
/************************* End of DataStore **************************/
